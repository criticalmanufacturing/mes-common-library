...
📦mes-common-library
 ...
 ┃ ┗ 📂feature_x_root_folder
     ┗ 📂 feature_x_subpackage_1
     ┗ 📂 feature_x_subpackage_2
     ┗ 📂 libs
     ...
...
- **`scaffolding/`:** This directory contains helper scripts and supporting configuration files used to bootstrap or automate repository-specific setup tasks in a consistent way. Currently the following is supported:
  - `scaffolding/new-feature.sh` - is specifically focused on creating a new CM MES Community customization project, its first feature, and the standard customization layers.
...
All features **_MUST_** contain tests and documentation. The tests should be unit tests, or mocked tests, **_they must not depend_** on an MES instance. Every feature must be self describable. This means it should contain all the necessary code and documentation for it to be used. All features, will follow the CLI pipeline of `cmf restore`, `cmf build` and `cmf package`.

If a feature does not contain tests and documentation it must be rejected. If a feature does not have enough test coverage or has missing documentation it will be rejected.
...
All CLI Packages must follow the format:
...
The version will contain in the major the CM MES version plus the major, followed by minor and patch. For example:

- If a package applies to version 11.3.0 of CM MES and is in the version 1.0.0 of that package, it should have the version `11301.0.0`.
- If in version 11.3.0 and the package suffers a breaking change, it will move to version `11302.0.0`.
- If the package is updated to version 11.3.1 and no change is done beyond update it should move to version `11311.0.0`.
- If the package is in version 11.3.1 and receives bug fixes, it should move to version `11311.0.1`.
...
```
{FEATURE_NAME}/{MES_MAJOR}_{MES_MINOR}[_{MES_PATCH}]-v{MAJOR}_{MINOR}_{PATCH}
```

### Components

- **`{FEATURE_NAME}`**: The name of the feature the tag is targeting. Matches the feature name/root folder used in the repository.
...
| Tag                          | Description                                                      |
| ---------------------------- | ---------------------------------------------------------------- |
| `FeatureA/11_1_0-v11101_0_0` | Version 1.0.0 of a feature for MES v11.1.0                       |
| `FeatureA/11_2_0-v11201_0_0` | Version 1.0.0 of a feature for MES v11.2.0 (Upgrade)             |
| `FeatureA/11_2_0-v11201_1_0` | Version 1.1.0 of a feature for MES v11.2.0 (Non-breaking change) |
| `FeatureA/11_2_0-v11202_0_0` | Version 2.0.0 of a feature for MES v11.2.0 (Breaking change)     |
| `FeatureA/11_1_0-v11101_0_1` | Version 1.0.1 of a feature for MES v11.1.0 (Bug fix)             |
| `FeatureA/11_2_0-v11202_0_1` | Version 2.0.1 of a feature for MES v11.2.0 (Bug fix)             |
| `FeatureA/11_3_0-v11301_0_0` | Version 1.0.0 of a feature for MES v11.3.0 (Upgrade)             |

**Important Note**: Package versions can evolve independently across MES versions. The same feature may have different major versions for different MES releases, reflecting the changes needed to adapt to each MES version.

### Version Compatibility

- A library version tagged for a specific MES patch (e.g., `FeatureA/11231_0_0`) is compatible only with that patch version.
- Each MES version maintains its own version sequence - versions are not comparable across different MES versions.
- Always use the latest library version available for your specific MES version.

## 
Release Pipelines

The following release pipelines are available for managing each feature's development cycle and their releases:

| Action    | Description                                                                                                                                                                                                          |
| --------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `PR`      | Runs when a pull request is opened or updated for supported branches. It validates that changed packages can be built (while also running unit tests), without packing or promoting them.                            |
| `CI`      | Runs manually or when changes are pushed to development, fix, or feature branches. It builds, packs, and tests changed packages, then promotes the tested packages to the persistent CI package store.               |
| `Release` | Runs manually for one feature or all features. It retrieves packages already built and tested by CI, performs a final sanity check on test packages, and publishes the feature artifacts in a tagged GitHub Release. |
n.
...
If the package that you are importing contains a Business package and you want access to those `.dll`s. Add the package Business package to the Business layer's `cmfpackage.json` of your project and add the required assemblies as NuGet references:
...
This project is licensed under the [BSD 3-Clause License](LICENSE.txt) - see the [LICENSE.txt](LICENSE.txt) file for details. _(Replace with the actual license)_
