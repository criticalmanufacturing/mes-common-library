...
- **`pre-commit`** — runs `.github/scripts/validate-package-versions.sh`, rejecting the commit if any `cmfpackage.json` version doesn't follow the [`{MES_MAJOR}{MES_MINOR}{MES_PATCH}{MAJOR}.{MINOR}.{PATCH}` rule](README.md#features-versioning).
