...
namespace Cmf.Connect.IoT.Driver.MTConnect.DriverObjects
{
    ...
    public class EventDispatcher
    {
        ...
        {
            Shared.Log.Debug($"TriggerEvent {eventData.Header.InstanceId}");
            var callback = this.m_EventListeners.GetValue((string)eventData.EventId.ToString(), null);
            if (callback != null)
            {
                Shared.Log.Debug($"Send to Controller TriggerEvent {eventData.Header.InstanceId}");
                callback(eventData.ToJson());
            }
            Shared.Log.Debug($"TriggerEvent Completed {eventData.Header.InstanceId}");
        }
        ...
        {
            this.m_EventListeners = new ConcurrentDictionary<string, Func<object, Task<object>>>(StringComparer.InvariantCultureIgnoreCase);
        }
    }
}
