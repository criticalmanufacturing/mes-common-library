...
import { MTConnectCommunicationSettings } from "../communicationSettings";
...
@injectable()
export class MTConnectHandler extends EventEmitter {
    protected _configuration: Configuration;

    private _container: Container;
    private assemblyName: string = path.join(__dirname, "../../lib/Cmf.Connect.IoT.Driver.MTConnect.dll");
    private typeName: string = "Cmf.Connect.IoT.Driver.MTConnect.Startup";
    private _pJson: any = {};
    ...
    // Parameter is required to fix error The number of constructor arguments in the derived class MTConnectHandler must be >= than the number of constructor arguments of its base class.
    public constructor(@inject("dummy") private dummy: any) {
        super();
    }

    /**
     * Initialize the MTConnectHandler. Initialized at the same time as the driver implementation
     */
    ...
    public async initializeEdge(): Promise<void> {
        // Make sure .net core is being used
        process.env["EDGE_USE_CORECLR"] = "1";
        // Surface which native edge*.node file edge-js decides to load, and from where
        process.env["EDGE_DEBUG"] = process.env["EDGE_DEBUG"] || "1";
        try {
            this._logger.info(
                `Initializing edge-js. platform=${process.platform} arch=${process.arch} ` +
                `nodeVersion=${process.version} cwd=${process.cwd()}`
            );

            // Validate that the edge_coreclr.node is in lib/native folder
            const edgeCoreClrPath = path.join(__dirname, "../../lib/native/edge_coreclr.node");

            // Validate that the hash of the edge_coreclr.node is the same as one of the hashes of the edge_coreclr under /workspaces/mes-common-library/features/IoTMTConnect/IoTMTConnect.IoT/src/driver-mtconnect/EdgeJs/22.1.2
            ...
            this._logger.error(`Unable to initialize Edge: ${MTConnectHandler.describeError(error)}`);
            throw new Error(`Unable to initialize Edge: ${MTConnectHandler.describeError(error)}`);
        }
    }

    /**
     * Builds a detailed, loggable description of an error, including properties that a plain
     * `${error}` template interpolation drops (stack trace, and any extra fields Node attaches
     * to native module load failures, such as `code`, `errno` or `path`).
     */
    private static describeError(error: any): string {
        ...
        const extraProps = Object.getOwnPropertyNames(error)
        ...
        const extra = Object.keys(extraProps).length > 0 ? ` | extra=${JSON.stringify(extraProps)}` : "";
        return `${error.stack || error.message}${extra}`;
    }
     ...
     */
    public async connect(communicationSettings: MTConnectCommunicationSettings): Promise<void> {
        this._communicationSettings = communicationSettings;
        this._logger.info(`*********************************************************** 21`);

        return (new Promise<void>((resolve, reject) => {
            this._logger.info(`*********************************************************** 22`);
            this._driver_connect(communicationSettings, async (error: any, result: any) => {
                if (error) {
                    this._logger.info(`*********************************************************** 23`);
                    reject(error);
                } else {
                    this._logger.info(`*********************************************************** 24`);
                    if (result === false) {
    ...
    public async registerEvent(eventId: string): Promise<boolean> {
        return new Promise<boolean>((resolve, reject) => {
            if (this._driver_registerEvent == null) {
                this._logger.warning(`MTConnect handler is not yet initialized, so the event '${eventId}' will be registered afterwards`);
    ...
    public async registerDotNetAssemblyHandlers(): Promise<void> {
        ...
        this._logger.info(`Registering DotNetCore assembly functions with '${assemblyPath}'`);
    ...
    public async setConfiguration(configuration: Configuration, communication: MTConnectCommunicationSettings): Promise<void> {
        ...
        if (driverDefinitions && driverDefinitions.events && driverDefinitions.events.length > 0) {
            for (const equipmentEvent of driverDefinitions.events) {
                if (equipmentEvent.isEnabled) {
                    ...
                    try {
                        this._logger.info(`Registering event '${equipmentEvent.name}' (${equipmentEvent.systemId}) on MTConnect`);
    ...
    private async onEventOccurrence(occurrence: any, callback: any): Promise<void> {
        ...
        this.emit("onMTConnectEventOccurrence", occurrence.eventId, occurrence);
