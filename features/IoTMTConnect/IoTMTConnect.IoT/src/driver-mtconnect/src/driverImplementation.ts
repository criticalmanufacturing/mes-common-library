...
export class MTConnectDeviceDriver extends DeviceDriverBase {
    ...
    private async onEventOccurrence(
        ...
        this.logger.debug(`Event Occurrence received for event '${eventId}' with values: ${JSON.stringify(evtOccurValue)}`);

        // Check if there are any custom events registered for this eventId
        ...
        if (registeredEvents.length > 0 && evtOccurValue) {
            this.logger.debug(`Custom Event '${eventId}'`);
        ...
        if (event && event.isEnabled) {
            this.logger.debug(`Default Event '${eventId}'`);
            const results: PropertyValue[] = [];
            ...
            this.logger.debug(`Default Event '${JSON.stringify(event)}'`);
