...
describe("CodeExecution Api (messageBus) tests", () => {
    ...
    */
    ...
    beforeEach(() => {
        ...
        messageBusMock = new MessageBusMock();
    });
    ...
    const codeExecutionTestFactory = (customSettings: any | undefined,
        ...
        EngineTestSuite.createTasks([
            ...
            (container: Container) => {
                ...
                container.unbind(TYPES.System.MessageBus);
                container.bind(TYPES.System.MessageBus).toConstantValue(messageBusMock);
            });
    };

    it("should be able to publish a value", (done) => {
        const code = codeTemplate.replace("<CODE>", `
            const retrievedValue = await this.framework.messageBus.publish("testingSubject", "Hello World");
        ...
        codeExecutionTestFactory({ jsCodeBase64: encode(code), },
            (values: Map<string, Task.Output<any>>) => {
                messageBusMock.subscribe("testingSubject", (subject: string, body: any) => {
                    if (subject === "testingSubject" && body === "Hello World") {
    ...
    it("should be able to send a value and receive a reply", (done) => {
        const code = codeTemplate.replace("<CODE>", `
            const retrievedValue = await this.framework.messageBus.sendRequest("testingSubject", "Hello");
        ...
        codeExecutionTestFactory({ jsCodeBase64: encode(code), },
            (values: Map<string, Task.Output<any>>) => {
                ...
                messageBusMock.subscribe("testingSubject", (subject: string, body: any, reply: any) => {
                    if (subject === "testingSubject" && body === "Hello") {
                        reply("World");
                    }
                });
            ...
            (changes: Task.Changes) => {
                try {
                    expect(changes["success"].currentValue).to.equal(true);
                    expect(changes["input1"].currentValue).to.equal("World");
