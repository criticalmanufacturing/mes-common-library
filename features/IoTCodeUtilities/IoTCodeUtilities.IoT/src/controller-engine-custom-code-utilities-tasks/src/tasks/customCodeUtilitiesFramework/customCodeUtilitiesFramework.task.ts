...
import { CustomCodeUtilitiesFramework, ID } from "./customCodeUtilitiesFramework.task.util";
 ...
 */
@Task.Task()
export class CustomCodeUtilitiesFrameworkTask extends TaskBase {
    ...
    public override async onBeforeInit(): Promise<void> {
        if (this.taskCodeExecutionLibs != null) {
            if (this.taskCodeExecutionLibs.implementations[ID] == null) {
                this.taskCodeExecutionLibs.addImplementation(ID, new CustomCodeUtilitiesFramework());
            }
        }
    }
}