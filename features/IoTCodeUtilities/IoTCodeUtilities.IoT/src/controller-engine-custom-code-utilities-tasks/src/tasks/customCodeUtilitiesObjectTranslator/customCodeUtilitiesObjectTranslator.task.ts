...
import { CustomUtilitiesUtilObjectTranslator } from "./customCodeUtilitiesObjectTranslator.task.util";
    ...
    public override async onBeforeInit(): Promise<void> {
        if (this.taskCodeExecutionLibs != null) {
            if (this.taskCodeExecutionLibs.implementations[ID] == null) {
                this.taskCodeExecutionLibs.addImplementation(ID, new CustomUtilitiesUtilObjectTranslator());
            }
        }
    }
}
