#!/usr/bin/env bash
#
# Enforces two rules across every cmfpackage.json in the repo:
#
# 1. Every "packageId" is unique repo-wide (duplicates would make dependency
#    resolution and deployment ambiguous).
#
# 2. Package versioning follows:
#
...
#    dependency (e.g. "11.3.3"). The "version" of the root package and of
#    every cmfpackage.json nested under it must have a major segment that
#    starts with the MES version digits concatenated without dots (e.g.
#    "1133"), followed by the package's own major version (e.g. "113310.0.0"
#    is invalid, but "11331.0.0" and "113310" are consistent forms of MES
#    11.3.3 + package major 1 and 10 respectively).
set -euo pipefail

status=0

mapfile -t ALL_PACKAGES < <(find . -name cmfpackage.json -not -path "*/node_modules/*"  -not -path "*/Release/*" -not -path "*/bin/*" | sort)

declare -A PACKAGE_ID_FILES

for pkg_file in "${ALL_PACKAGES[@]}"; do
  package_id=$(jq -r '.packageId // empty' "$pkg_file")

  if [ -z "$package_id" ]; then
    echo "::error file=$pkg_file::Missing 'packageId'."
    status=1
    continue
  fi

  if [ -n "${PACKAGE_ID_FILES[$package_id]:-}" ]; then
    echo "::error file=$pkg_file::Duplicate packageId '$package_id', already declared in ${PACKAGE_ID_FILES[$package_id]}."
    status=1
  else
    PACKAGE_ID_FILES[$package_id]="$pkg_file"
  fi
done
...
for root_file in "${ALL_PACKAGES[@]}"; do
  ...
  mapfile -t GROUP_PACKAGES < <(find "$root_dir" -name cmfpackage.json -not -path "*/node_modules/*" -not -path "*/bin/*" | sort)
...
if [ "$status" -eq 0 ]; then
  echo "All cmfpackage.json packageIds are unique and versions conform to the {MES_MAJOR}{MES_MINOR}{MES_PATCH}{MAJOR}.{MINOR}.{PATCH} rule."
fi
